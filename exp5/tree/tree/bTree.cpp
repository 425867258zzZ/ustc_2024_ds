...
// 删除节点value=x 的节点及其子树
void deleteValue(bNode *root, int x)
{
	if (root)
	{
		...
		deleteValue(root->lchild, x);
		deleteValue(root->rchild, x);
	}
	return;
}

// 给定id1 和id2，输出其最近共同祖先id，节点的id 具有唯一性
int getParent(bNode *root, int id1, int id2)
{
	if (!root)
		exit(0);
	...
	bNode *node1 = locateByID(root->lchild, id1);
	bNode *node2 = locateByID(root->rchild, id2);
	if (node1 != NULL && node2 != NULL)
		return root->data.id;
	if (node1 != NULL && node2 == NULL)
		return getParent(root->lchild, id1, id2);
	if (node1 == NULL && node2 != NULL)
		return getParent(root->rchild, id1, id2);
	if (node1 == NULL && node2 == NULL)
	{
		printf("请检查输入");
		exit(0);
...
void getRoad(bNode *root, int id, std::queue<char> &queue)
{
	if (root->data.id == id)
		return;
	bNode *node1 = locateByID(root->lchild, id);
	bNode *node2 = locateByID(root->rchild, id);
	if (node1 != NULL && node2 == NULL)
	{
		queue.push('L');
		getRoad(root->lchild, id, queue);
	}
	if (node1 == NULL && node2 != NULL)
	{
		queue.push('R');
		getRoad(root->rchild, id, queue);
	}
	if (node1 == NULL && node2 == NULL)
	{
		printf("请检查输入");
...
#define MAXINT 99999
#define MININT -99999
int maxSub(bNode *root)
{
	static int max = MININT;
	static int min = MAXINT;
	static int sub = max - min;
	if (root)
	{
		...
		sub = maxSub(root->lchild);
		sub = maxSub(root->rchild);
	}
	return sub;
}
...
int main()
{
	...
	*/
	...
	printf("\nid1=9 和id2=10其最近共同祖先id:%d", getParent(t1, 9, 10));

	printf("\n树节点中最大的value 和最小的value 之差:%d", maxSub(t1));
	deleteValue(t1, 17);
	saveTree(t1, "sg.html");
}
